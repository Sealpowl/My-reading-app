...
const App = {
  ...
  openAISettings() {
    ...
    this.openModal(`
      ...
      <div class="modal-section">
        <div class="modal-label">选择服务商</div>
        <div class="status-btns">
          <div class="status-btn ${cur==='anthropic'?'active':''}" onclick="App._switchProviderTab('anthropic',this)">Claude</div>
          <div class="status-btn ${cur==='deepseek'?'active':''}" onclick="App._switchProviderTab('deepseek',this)">DeepSeek</div>
        </div>
        <input type="hidden" id="cur-provider" value="${cur}">
      </div>
      <div id="section-anthropic" class="${cur==='anthropic'?'':'hidden'}">
        <div class="modal-section">
          <div class="modal-label">Anthropic API Key</div>
          <input class="modal-input" type="password" id="key-anthropic" value="${escHtml(s.apiKey||'')}" placeholder="sk-ant-…">
        </div>
        <div class="modal-section">
          <div class="modal-label">模型</div>
          <select class="modal-select" id="model-anthropic">
            <option value="claude-sonnet-4-6" ${(s.model||'claude-sonnet-4-6')==='claude-sonnet-4-6'?'selected':''}>Claude Sonnet 4.6（推荐）</option>
            <option value="claude-haiku-4-5-20251001" ${s.model==='claude-haiku-4-5-20251001'?'selected':''}>Claude Haiku 4.5（更快）</option>
          </select>
        </div>
      </div>
      <div id="section-deepseek" class="${cur==='deepseek'?'':'hidden'}">
        <div class="modal-section">
          <div class="modal-label">DeepSeek API Key</div>
          <input class="modal-input" type="password" id="key-deepseek" value="${escHtml(s.deepseekKey||'')}" placeholder="sk-…">
        </div>
        <div class="modal-section">
          <div class="modal-label">模型</div>
  ...
  _switchProviderTab(pid, btn) {
    ...
    document.getElementById('section-anthropic').classList.toggle('hidden', pid !== 'anthropic');
    document.getElementById('section-deepseek').classList.toggle('hidden', pid !== 'deepseek');
  },

  saveAISettings() {
    Store.update(d => {
      d.settings.provider = document.getElementById('cur-provider').value;
      d.settings.apiKey = document.getElementById('key-anthropic')?.value?.trim() ?? d.settings.apiKey;
  ...
  async _sendAIMessage(discId) {
    ...
    const s = data.settings;
    const provider = s.provider || 'anthropic';
    const apiKey = provider === 'anthropic' ? (s.apiKey || '') : (s.deepseekKey || '');
    const model = provider === 'anthropic'
      ? (s.model || 'claude-sonnet-4-6')
      : (s.deepseekModel || 'deepseek-chat');
    if (!apiKey) { this.toast('请先设置 API Key', 'error'); return; }
    ...
    const msgs = disc.messages.map(m => ({ role: m.role, content: m.content }));
    ...
    try {
      ...
      if (provider === 'anthropic') {
        const res = await fetch('https://api.anthropic.com/v1/messages', {
          ...
          body: JSON.stringify({ model, max_tokens: 1000, system: systemPrompt, messages: msgs })
        });
        ...
        const res = await fetch('https://api.deepseek.com/chat/completions', {
          ...
          body: JSON.stringify({ model, max_tokens: 1000, messages: [{ role: 'system', content: systemPrompt }, ...msgs] })
        });
        if (!res.ok) { const e = await res.json().catch(()=>({})); throw new Error(e.error?.message || `${res.status}`); }
        const json = await res.json();
