...
const App = {
  ...
  openAISettings() {
    ...
    const providers = [
      { id: 'anthropic', label: 'Claude',    placeholder: 'sk-ant-…',
        models: [['claude-sonnet-4-6','Claude Sonnet 4.6（推荐）'],['claude-haiku-4-5-20251001','Claude Haiku 4.5（快）']] },
      { id: 'openai',    label: 'ChatGPT',   placeholder: 'sk-…',
        models: [['gpt-4o','GPT-4o'],['gpt-4o-mini','GPT-4o mini（省）'],['o4-mini','o4-mini']] },
      { id: 'deepseek',  label: 'DeepSeek',  placeholder: 'sk-…',
        models: [['deepseek-chat','DeepSeek V3'],['deepseek-reasoner','DeepSeek R1']] },
      { id: 'gemini',    label: 'Gemini',    placeholder: 'AIza…',
        models: [['gemini-2.0-flash','Gemini 2.0 Flash'],['gemini-2.5-pro','Gemini 2.5 Pro']] },
    ];
    ...
    const sections = providers.map(p => {
      ...
      const modelOpts = p.models.map(([v,l]) => `<option value="${v}" ${modelVal===v?'selected':''}>${l}</option>`).join('');
      return `
        <div id="provider-section-${p.id}" class="${cur===p.id?'':'hidden'}">
          <div class="modal-section">
            <div class="modal-label">${p.label} API Key</div>
            <input class="modal-input" type="password" id="key-${p.id}" value="${escHtml(keyVal)}" placeholder="${p.placeholder}">
          </div>
          <div class="modal-section">
            <div class="modal-label">模型</div>
            <select class="modal-select" id="model-${p.id}">${modelOpts}</select>
          </div>
        </div>`;
    }).join('');
  ...
  _switchProviderTab(pid) {
    ...
    ['anthropic','openai','deepseek','gemini'].forEach(id => {
      document.getElementById('provider-section-' + id)?.classList.toggle('hidden', id !== pid);
    });
  },
